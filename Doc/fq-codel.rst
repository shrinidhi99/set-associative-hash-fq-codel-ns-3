...
The data for the graph can be generated by modifying the fq-codel-queue-disc.cc file and its corresponding test suite. FqCodel can be given as input hash values that are pre-generated using the python code and which is located in the hash folder. The test will output collision probability(sets filled/ total sets or queues filled/total queues) for either the set-associative hash or traditional hash depending on which is enabled. This can be output to console using print statements added to the fq-codel-queue-disc.cc file. Thirty such trials were conducted taking as input 30 different hash data sets, and then the average was calculated through the use of the spreadsheet. The averages of both were calculated, and the graph was plotted using spreadsheet tools. 
